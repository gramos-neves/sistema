	...
	private UserDetailsService users;
	    ...
	    throws Exception
	  {
	    ((HttpSecurity)
	    	    ...
	    	    ((ExpressionUrlAuthorizationConfigurer.AuthorizedUrl)
	    	    ((ExpressionUrlAuthorizationConfigurer.AuthorizedUrl)http.authorizeRequests().antMatchers(new String[] { "/api/public/**" }))
	    	    .permitAll().antMatchers(HttpMethod.POST, new String[] { "/api/login" }))
	    	    .permitAll().anyRequest())
	    		.authenticated().and()).csrf().disable())
	    .addFilterBefore(new StatelessLoginFilter("/api/login", this.tokenAuthenticationService, this.users, authenticationManager()), UsernamePasswordAuthenticationFilter.class)
